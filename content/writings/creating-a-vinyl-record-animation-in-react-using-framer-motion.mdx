...
Anyway, back to the topic! Inspired by that CD animation, I decided to create a spinning vinyl record component in React using **Motion** (the library formerly known as Framer Motion — it's now published as `motion` and imported from `motion/react`). Let’s build it together!

### Getting Started

This will be a simple component, so we’ll only need to install one package.

<InstallTabs pkg="motion" />
